...
### Fixed — переполненный контекст больше не убивает чат и не выдаётся за лимит подписки

- 🛠 **Симптом (25.09, чат владельца):** контекст 979153/1M → пользователю «Достигнут лимит подписки»,
  наш компакт «сжатие пропущено (лимит)», чат мёртв до `/clear`. Реальная квота не была исчерпана.
- 🔍 **Причина классификации:** локальный охранник CLI («Prompt is too long», $0, 0.1с; включается при
  `tokens >= window-3000`, ≈97.7%) завершает ход с `terminal_reason="blocking_limit"`. `send_message`
  считал это типизированным лимитом подписки и глушил ветку `context_limit`. Теперь текст переполнения
  побеждает `blocking_limit`; пустой `blocking_limit` без текста по-прежнему квота.
- ✅ **Самовосстановление:** если наш компакт падает именно с `context_limit`, `compact._native_compact_fallback`
  вызывает `ClaudeSession.native_compact` — родной `/compact` CLI в той же сессии (на проде руками:
  979153→2719 токенов; живая проверка через SDK на малой сессии: ок). Успех засчитывается только по
  системному сообщению `compact_boundary`; иначе честный терминал «нужен /clear». Исходное сообщение идёт
  тем же батчем. Люк открывается ТОЛЬКО по `context_limit`, не по любой ошибке.
- 🔍 **Почему рос до 979K:** превентивного и ночного компакта в коде нет (убраны в #34); единственный
  владелец — admission перед батчем при 95% (950K), а стена CLI ≈977K: одного тяжёлого хода хватило,
  чтобы перепрыгнуть. Порог `AUTO_COMPACT_TRIGGER_PCT` 95 → **88**, и после каждого отработавшего хода
  (`ChatState._finish_processing`) контекст меряется снова (только после реально прошедшего хода — упавший
  admission-компакт тут не повторяется).
- 🛠 `EXPECTED_MAX_OUTPUT_TOKENS` 64000 у Opus 5.5 давал ложный ERROR и латч на каждом ответе (реально 128000):
  теперь `expected_max_output_tokens(model)`.
- ⚠️ Известный компромисс: компакт срабатывает раньше (88%), на длинных чатах чуть чаще; нативная сводка
  не несёт наш дословный хвост и редакцию секретов.
