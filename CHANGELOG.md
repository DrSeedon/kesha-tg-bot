...
- 🛠 **Авария при переключении:** SDK 0.2.152 несёт встроенный Claude Code 2.1.259, а Opus 5.5 требует
  ≥2.1.280 → на каждое сообщение `API Error: 400 ... does not support this model`, и эта строка ушла
  пользователю как ответ. Откат модели, затем `claude-agent-sdk` 0.2.152 → **0.2.158** (встроенный CLI
  2.1.280), пин в `requirements.txt`. Сьют 601 зелёный на 0.2.158 + mcp 1.28.1, живой вызов через SDK ок.
...
- 🛠 **Симптом:** 21.09 в 14:33 Катя на голосовое получила единственную строку
  `The model's tool call could not be parsed (retry also failed)`. Для бота это выглядело успешным
  ответом (`tools=0, inner loop done, no retry needed`), потому что CLI отдаёт свою капитуляцию
  обычным ассистентским текстом, а не ошибкой.
- 🔍 **Механика по транскрипту:** модель выдала неразбираемый вызов инструмента → CLI сам подставил
  в диалог `The previous response failed to produce a valid tool call. Please retry the tool call now.`
  → второй заход тоже не разобрался (`stop_reason=stop_sequence`) → CLI ответил своей строкой.
  Обе фразы лежат в бинарнике Claude Code 2.1.278, то есть это поведение рантайма, не наш код.
- ✅ `is_tool_call_giveup` (`claude_session.py`) распознаёт её, `response_stream` уходит в уже
  существующий ретрай (до 2 попыток) и переиспользует тот же пузырь сообщения, чтобы английская
  строка заменялась ответом, а не висела над ним. Повторы кончились → сообщение по-русски.
- 📊 Частота: 1 случай на 9 928 сообщений в базе; у агентов Orchestra за всё время ноль.

## v2.9.2 — 2026-09-20

### Fixed — в учёт попадают токены субагентов и вспомогательных моделей

- 🧾 **Источник токенов — `ResultMessage.model_usage`, а не `usage`.** `usage` считает только вызовы
  самого агента: на живом замере 20.09 он показал out=912 / cache_create=11 296, тогда как полный
  счёт дал out=1443 / cache_create=28 157 за тот же ответ. Что важнее, сумма `costUSD` по моделям
  из `model_usage` совпадает с `total_cost_usd` до копейки — то есть это ровно тот счёт, за который
  списаны деньги. Видно было по отношению цены к токенам: обычные ответы дают 0.336–0.343, ответы
  с субагентом давали 0.476–0.482.
- ➖ **`model_usage`, как и стоимость, накопительный по сессии** (проверено: out 399 → 402 после
  ответа в 3 токена), поэтому пишется дельта; откат счётчика трактуется как цена ответа.
- 🛟 Результат без `model_usage` (терминальные ошибки) откатывается на `usage`, иначе спенд ответа
  потерялся бы совсем. Метаданные модели без единого счётчика токенов за счёт не принимаются.
- ⚠️ Известный компромисс: колонки `cache_creation_5m_tokens` / `cache_creation_1h_tokens` читаются
  из `usage` — разбивки TTL в `model_usage` нет. Значит сумма кеш-записи покрывает всё, а её
  разбивка — только главного агента.

## v2.9.1 — 2026-09-20

### Fixed — учёт стоимости считал не то (найдено приёмкой, не тестами)

- 💰 **`total_cost_usd` от SDK — накопительный расход сессии, а не цена ответа.** Писался как есть,
  и строки росли 5.17 → 6.26 → … → 47.94 за сессию. Замер на живом запросе: ответ из 3 выходных
  токенов «стоил» 0.0279 сразу после ответа из 165 токенов за 0.0155. Теперь в `response_usage`
  пишется дельта; откат счётчика (новая сессия CLI) трактуется как цена ответа. Побочно исправлен
  `ChatSession.total_cost_usd`, который складывал накопительные значения и рос квадратично.
- 🔢 **`usage` в сообщениях ассистента — снимок на СТАРТЕ генерации.** На ответе в 242 выходных
  токена там стоит 6; по строкам это давало 93 вместо 4 348. Токены ответа теперь берутся из
  терминального `ResultMessage.usage` и складываются по ретраям внутри одного ответа.
- 📏 **`context_tokens` был суммой по вызовам, а не размером контекста**: на трёхвызовном ответе
  1 459 171 против реальных 730 857. Колонка берётся из последнего вызова (снимок ассистента,
  у которого входная сторона точна). В строках, записанных до фикса, колонка обнулена в NULL.
- ✅ Сверка с транскриптом Claude CLI на пяти ответах (1 и 2 вызова): output, cache_read,
  cache_creation и input совпадают дословно. Все три денежные ветки закрыты тестами с мутацией.

## v2.9.0 — 2026-09-19

### Added — стоимость и токены каждого ответа в sqlite (`response_usage`)

- 💰 **Новая таблица `response_usage` в `messages.db`**: время, `chat_id`, рантайм, модель,
  `num_turns`, длительность, `input` / `cache_creation` (+ разбивка `ephemeral_5m` и `ephemeral_1h`) /
  `cache_read` / `output` токены за весь ответ, размер контекста последнего вызова и `total_cost_usd`
  от SDK. Запись переживает рестарт и `/clear`; накопитель в памяти (`/status`) источником больше не считается.
- 🧮 **Токены считаются ОДИН раз на `message.id`.** Поток повторяет одно и то же сообщение модели
  на каждый блок содержимого вместе с его `usage`: на боевом транскрипте 454 записи против 260
  настоящих вызовов (173 id с побайтово одинаковым `usage`). Наивная сумма завышала счёт в ~1.7 раза.
- 🔁 Ретрай внутри одного ответа складывается в ту же строку: повторный вызов реально потрачен.
- ⬜ Чего рантайм не сообщает — пишется NULL, не ноль: у Codex нет стоимости и разбивки cache-write.
- 🔌 `reset_response_usage` добавлен в `ChatRuntime` и `_REQUIRED_METHODS`: новый рантайм не сможет
  молча не заводить учёт и выставить счёт следующего ответа предыдущему пользователю.

### Fixed — логи рантаймов не терялись молча

- 🪵 `claude_session` и `codex_session` брали `logging.getLogger(__name__)`, а обработчики висят
  только на логгере `kesha` → весь их INFO глушил `logging.lastResort` (WARNING+). Строка
  `Result: …s, N turns, stop=…, cost=$X` не попадала НИКУДА: 0 совпадений в journal за 28 суток
  и 0 в `logs/kesha.log*`. Теперь оба — дети логгера `kesha`.

## v2.8.1 — 2026-09-15

### Fixed — в пузыре статуса видно, КАКОЙ скилл запущен

- 🧩 `Skill` рисовался как `🔧 Skill · 2s` без имени: `_format_hint` (`tool_status.py`) искал
  подсказку в ключах `command/file_path/query/...`, а тул кладёт имя в `skill` — под фильтр не
  попадало ничего. Теперь `skill` проверяется первым (иначе имя перекрыл бы `args`), и у тула
  свой значок: `🧩 Skill youtube-deep · 2s`.
...
### Fixed — Текст больше не слипается через границу тула (#27)

- 🩹 **Что.** Предложение до вызова тула и предложение после него уезжали одним сообщением без
  разделителя: «…найду топ по карме.Нашёл главное:».
- 🔧 **Техсуть.** `response_stream.py`: `_finalize_text_block()` вызывался только на `turn_done`
  и в конце потока, а в ветке `ct == "tool"` финализации не было — `parts` не сбрасывался на
  границе, и следующий текстовый блок дописывался к предыдущему. Теперь текстовый пузырь
  закрывается ДО открытия пузыря тулов, а новый `_open_text_block()` симметрично закрывает пузырь
  тулов перед новым текстом. Порядок в чате стал хронологическим, как в Claude Code:
  текст → пузырь → текст → пузырь.
- 🗑 **Removed.** Механика переезда текста под пузырь из #28 (`text_needs_move` и её ветки в
  `_edit_update` / `_finalize_text_block`) стала недостижимой и удалена целиком.
- ↩️ **Known tradeoff.** Это сознательная отмена раскладки #28 («один текстовый пузырь и один
  пузырь тулов сверху») по решению владельца: склейка блоков была её прямым следствием. Тулов
  теперь может быть несколько пузырей за ход — по одному на каждую серию вызовов.
- 🧪 **Triggered case.** Юзер прислал скриншот со слипшимся «карме.Нашёл». Тест
  `test_stream_layout.py::test_text_blocks_never_glue_across_a_tool` мутационно проверен: снятие
  любой из двух финализаций красит его.

## v2.7.0 — 2026-08-03

Второй рантайм (Codex) как аварийный вариант, ночной автокомпакт с резервом контекста,
честные отказы вместо молчания. Дефолт не изменился — `KESHA_RUNTIME` не задан = Claude,
как раньше. Обратной несовместимости нет, поэтому минор, а не мажор.

### Added — Два рантайма: Claude + Codex, ручное переключение (#16)

- 🧩 **Команда `/runtime`.** Без аргумента — текущий рантайм, его модель, остаток квоты и дата
  сброса. `/runtime codex` / `/runtime claude` — переключить ЭТОТ чат. Переключение только когда
  бот свободен (`is_busy` → «дождись ответа или /stop»). Доступна всем из `ALLOWED`: в проекте нет
  понятия владельца, а общий бюджет закрывается ВИДИМОСТЬЮ — каждое переключение объявляет себя в
  чате. Причина записана в докстринг `h_runtime`, чтобы никто не «починил» обратно.
- ⚠️ **Это аварийный вариант, а не равноправный режим.** Автопереключения нет (T8 не делался),
  реального пользовательского пробега на Codex не было, квота Codex выжжена тестами до 08.08.
  Дефолт остаётся Claude.
- 🔌 **Техсуть — узкий протокол, толстые адаптеры:** `runtime_protocol.py` (`ChatRuntime`,
  `RuntimeCapabilities`), `runtime_registry.py` (fail-loud сборка: нет метода / сигнатура не
  сходится / capabilities врут → падаем при сборке, не в проде), `codex_session.py` (адаптер
  `codex app-server` по JSON-RPC), `chat_state.py` (`switch_runtime`, `_probe_runtime`,
  `_deliver_handoff`, `ChatRegistry.build_session`). Компакт в протокол НЕ входит — диспетчер по
  `native_compact` в вызывающем слое.
- 💾 **Файлы сессий по рантайму:** `storage/sessions/<chat_id>` (claude, байт в байт как было) и
  `<chat_id>.codex`. Возврат на рантайм продолжает ЕГО историю, а не чужую.
- 🤝 **Хендофф выжимкой.** При переключении недавняя история отдаётся новому рантайму текстом
  (`HANDOFF_MESSAGE_LIMIT=40`, `HANDOFF_MESSAGE_CHARS=2_000`, `HANDOFF_MAX_CHARS=24_000`). Не
  поднялся целевой рантайм → бот говорит об этом В ЧАТ и остаётся на текущем: молчаливый фолбэк
  противоречит fail-loud, юзер не поймёт, кто ему отвечает.
- 🔐 **Мост MCP-тулов для Codex** (`tool_bridge.py`): unix-сокет + capability-токен, TTL 3600с,
  адресация по сессии моста (`X-Kesha-Bridge-Session`), `revoke_chat_sessions(chat_id, runtime)`.
  15 тулов за мостом, `run_on_laptop` в `BRIDGE_EXCLUDED` НАВСЕГДА.
- 🛡 **Песочница отправки файлов** (`file_access.py`): `resolve_sendable` / `open_sendable`,
  `KESHA_SENDABLE_ROOTS`, `MAX_SEND_BYTES=50MB`.
- 🆕 **Env:** `KESHA_RUNTIME` (дефолт `claude`), `KESHA_CODEX_MODEL` (`gpt-5.6-sol`),
  `KESHA_CODEX_BIN`, `KESHA_CODEX_HOME`, `KESHA_SENDABLE_ROOTS`, `KESHA_BRIDGE_SOCKET`.
  Справка по выкатке: `docs/tasks/16/deploy-notes.md`.
- 🧪 Тесты: `test_runtime_protocol.py` (12), `test_runtime_registry.py` (8), `test_tool_bridge.py` (39),
  `test_bridge_tools.py`, `test_bridge_chat_routing.py`, `test_file_access.py` (27),
  `test_laptop_command_guard.py`, `test_codex_session.py` (43), `test_runtime_switch.py` (21),
  `test_compact_dispatch.py` (12), `test_runtime_limits.py` (17).
- **Triggered case:** бан или лимит подписки Claude = бот молчит целиком, запасного пути нет.
  Нужен второй рантайм, на который можно уйти рукой, не переписывая бота.

### Fixed — Дыры, найденные и закрытые по ходу #16

- 🕳 **TOCTOU в отправке файлов.** `FSInputFile` запоминает путь и читает ПОЗЖЕ → подмена на
  симлинк после проверки отдавала `.env`. Fix: `open_sendable()` читает в момент проверки
  (`O_NOFOLLOW`), тулы перешли на `BufferedInputFile`. **Урок методики: проверять конечное
  действие (что реально ушло в Telegram), а не промежуточную функцию.**
- 🔗 **Hardlink обходил whitelist** — у жёсткой ссылки нет «цели», путь резолвится сам в себя,
  `resolve()` бессилен. Fix: при `st_nlink>1` все имена файла обязаны лежать внутри корней.
- 🎭 **Фильтр `chat_id` пробивался пятью написаниями** — ` chat_id`, `chat_id `, `chat-id`,
  кириллическая `с`, ZWSP. Fix: whitelist аргументов из `input_schema` тула + `normalize_arg_name`
  (отказ на не-ASCII ДО нормализации). Whitelist вместо blacklist.
- 📨 **Межчатовая утечка адресации.** Процессная глобаль `_active_chat_id` перезаписывалась при
  двух активных юзерах → ответ мог уйти не тому. Fix: адресация через сессию моста + `copy_context()`
  на вызов. На проде дыры не было — глобаль жила только в ветке.
- 🧨 **`-c mcp_servers={}` НЕ РАБОТАЛ** — выглядел как изоляция, не делал ничего (мерджится в
  конфиг юзера): 5 глобальных MCP-серверов стартовали в треде Кеши. Fix: приватный `CODEX_HOME` +
  `--disable apps`. Замер: было `['serena','kwin','orchestra',...]`, стало `[]`, с мостом `['kesha']`.
- 🧟 **Хвост прерванного turn утекал в следующий ответ** — `/stop`, затем новый вопрос получал
  чужой текст. Fix: `_discard_turn_events(turn_id)`.
- 🗜 **Компакт на Codex рапортовал ok и НЕ РАБОТАЛ** (14174 → 14204 токена, контекст РОС).
  `thread/compact/start` сперва ПРЕРЫВАЕТ текущий turn (свой `turn/completed{interrupted}`) и лишь
  потом открывает ВТОРОЙ turn с `contextCompaction` — цикл выходил на первом. После фикса
  14198 → 14185. Поймано живым прогоном, не моком.
- 🔑 **Симлинк `auth.json` висел после logout** — молчаливая потеря авторизации.

### Fixed — Прод-инцидент 01.08: Кеша замолчал (#14)

- 🚨 **Симптом:** на каждое сообщение «⚠️ Не удалось проверить свободный контекст». Бот был жив,
  но не отвечал НИКОМУ и не чинился сам — только `/clear`, рестарт процесса или правка состояния
  извне.
- 🔍 **Root cause не тот, что казался.** Модель и `[1m]` были верны (замер на проде:
  `maxTokens=1000000`). Настоящая причина: Result события квоты приходит с `is_error=False` и
  пустым/частичным `model_usage` (например только легитимная запись Haiku, без Opus) → проверка
  инварианта латчила `_max_output_tokens_valid=False` **навсегда** → все последующие сообщения
  умирали в МОЛЧАЛИВОЙ ветке `runtime_invariant`, поэтому в логах не было ничего.
  **Событие квоты классифицировалось как вечная поломка рантайма.**
- 🔧 **Фикс:** классификация лимита (`result_is_limit`, `limit_seen`, `result_is_context_limit`)
  идёт ТЕПЕРЬ ПЕРЕД валидацией терминального usage (`claude_session.py:407`); пустой usage не
  латчит; латч снимается хорошим payload; лимит и контекст-лимит короткие замыкания инвариант не
  трогают; юзеру называется настоящая причина. `expected_context_model` выводится из `config.MODEL`,
  а не хардкодится. Коммиты `ad81cd5`, `e0b1d4e` — 7 файлов, +671/−38.
- 🧪 **167 зелёных тестов при мёртвом проде.** Фикстуры гоняли `claude-sonnet-4-6`, а не боевую
  `config.MODEL` — баг был вне зоны их видимости. Теперь `make_session` строит сессии ИЗ КОНФИГА,
  тесты mutation-проверены (вернуть баг → краснеют). `tests/test_claude_session_limit.py`.
- **Triggered case:** юзер написал боту утром — и получил одну и ту же ошибку на каждое сообщение,
  включая после рестарта сервиса.

### Added — Автокомпакт только ночью + резерв контекста (#14)

- 🌙 **Нативный auto-compact выключен** (`env={"DISABLE_AUTO_COMPACT": "1"}`, `claude_session.py:253`),
  у компакта теперь ОДИН владелец. Автоматическое сжатие — только в окне **23:00–08:00** Красноярск
  (`_is_auto_compact_night`, `_seconds_until_night`; проверено по всем 24 часам: окно = локальные
  часы `[0..7, 23]`).
- 🚧 **Fail-closed резерв контекста днём** (`check_context_reserve`): не хватает места — сообщение
  НЕ уходит в модель, контекст не растёт, Кеша один раз просит `/compact`. База допуска **208 000
  токенов** плюс длина собранного промпта в байтах UTF-8. Пол ручного компакта — **80 000** токенов
  (замеренный минимум запаса под свою выжимку 65 622 + 20% неопределённости и округление вверх).
- 🔓 **Латч резерва снимается и `/clear`, и успешным компактом** → не залипает.
- 🗄 Аддитивная миграция `chat_activity`. Удалены `run_native_manual_compact` и
  `compact.maybe_auto_compact()`.
- **Triggered case:** нативный `/compact` через `query("/compact ...")` возвращал успешный Result
  БЕЗ `compact_boundary(trigger="manual")` (`missing_manual_boundary`) — то есть «успех» ничего не
  доказывал. Раз доказательства сжатия нет, компактом должен владеть наш код.

### Fixed — Стриминг замирал при флуд-контроле Telegram (#17)

- 🧊 **Симптом:** пишешь боту во время стрима → правки ответа замирают, потом всё дописывается
  разом. Генерация при этом шла нормально — замирало только отображение.
- 🔧 **Причина и фикс:** потолок Telegram ~20 правок/мин на чат, а бюджет считался на пузырь.
  `STREAM_EDIT_INTERVAL = 1.0` заменён на общий на чат `_ChatEditBudget` с
  `CHAT_EDIT_INTERVAL = 3.1`; при флуд-лимите — фолбэк send+delete
  (`STREAM_FLOOD_SEND_INTERVAL = 3.0`, `_replace_live_message`) вместо заморозки. Плюс дедуп
  видимого среза и пропуск правки, если финальный текст не изменился. `response_stream.py` (+142/−26),
  `tests/test_response_limit.py` (4 теста). Codex: APPROVED.
- ❗ **Это НЕ регрессия #14** — механизм предсуществующий; гипотеза про `session.inject` опровергнута
  (инжект не вызывается ни из одного ingress-пути после #14).

### Fixed — Минутное «печатает» вместо честного отказа (#20)

- ⏱ **Таймаут контрол-запроса 60с → 10с + один ретрай.** Проба контекста наследовала дефолтный
  бюджет SDK (60с), поэтому зависший процесс держал юзера минуту, а потом отказывал с
  `reason="unknown"` и советом «отправь ещё раз» — повтор попадал в тот же залипший процесс ещё на
  60с. `CONTEXT_PROBE_TIMEOUT_S=10` (≈3× худшей здоровой латентности: здоровый запрос 0.9–3.4с),
  `_ProbeTimeout`, `_probe_context_usage()`, реконнект после ВТОРОГО подряд таймаута.
- 🩺 **Новая честная причина `runtime_unhealthy`** вместо `unknown` (`config.py`, ru+en).
- 💧 **Утечка `pending_control_responses`** (замер 0→1→2→3, не освобождались) — после фикса обратно 0.
- 📊 E2E-замер: `wedged reserve: 20.0s -> ok=False reason=runtime_unhealthy (было 60s)`,
  `after reconnect: 7.8s -> ok=True`, session id сохраняется. Тесты: 189 (было 181), 7 новых.
- **Triggered case:** прод-лог показал пары ровно в 60.0с — `10:06:26` и `10:07:26`
  `Control request timeout: get_context_usage` — то есть юзер минуту смотрел на «печатает».

### Changed — Промпт сжатия: обе полярности, ноль записей, дословный хвост (#21)

- ⚖️ **Запрет неподтверждённых утверждений в ОБЕИХ полярностях.** Промпт банил только позитивное
  выдумывание, а в проде выжимки писали и негативное: хрестоматийный случай —
  `` `CLAUDE.md` — Not modified this session ``. Замер 18 боевых выжимок: **7 инвертированных
  утверждений, 6 из них легитимны** → добавлено значение `UNKNOWN — source gap`.
- 🚫 **Явный выход на ноль для записи файлов**: «Otherwise do not write any file. Never create a new
  note, CLAUDE.md, TODO.md, or BUGS.md solely for compaction.» Аудит: 22 окна сжатия → 2 записи, 1
  уникальный файл.
- 📌 **Дословный хвост гарантируется КОДОМ, а не просьбой к модели** — `append_verbatim_tail` +
  `_recent_user_rows()`, маркер `[VERBATIM TAIL — appended by runtime]`. Наивный «последние 3»
  промахивался: 17% строк с `role=user` — напоминалки и системные, `18/57 = 32%` окон содержали
  не-юзерскую строку; на текущем боевом окне наивный вариант давал 2 ошибки атрибуции, наш — 0.
- 🎯 **Взяты выводы Orchestra #106, но НЕ их цель «короче».** Замер на своих данных: медианная
  выжимка 2058 токенов = **0.206% окна** и 4.6% пола после сжатия — ужать её вдвое дало бы 0.1%
  контекста. «Короче» — их ограничение, не наше; сокращение 3601→1500 снято с повестки.
- 🐛 Найдено в своём же коде до коммита: `get_history` идёт `ORDER BY id DESC`, поэтому `users[-limit:]`
  брал САМЫЕ СТАРЫЕ три; `message_id` бывает `None`, не только `0` (`TypeError` на живых данных).
- `compact.py` (+64/−4), `chat_state.py` (+18), `tests/test_compact_prompt.py` (9 тестов).
  Всего 198 тестов (было 189).

### Added — Гайд клиенту по регистрации и оплате (#15)

- 📄 `docs/tasks/15/guide-alexander.md` — разбор пяти подряд банов аккаунта. Первичная причина НЕ
  датацентровый IP (рабочая гипотеза, опровергнута — наш бот месяцами живёт на DC-IP Contabo):
  РФ отсутствует в Supported Regions, а убивает СТЕК сигналов — неподдерживаемый регион + прыгающий
  VPN + покупная почта + оплата через минуты после регистрации + сразу бот.
- ✅ Заодно проверено по первоисточнику: **Agent SDK на подписке легитимен, доплачивать не надо** —
  анонс про отдельный пул кредитов с 15.06.2026 отменён в день вступления в силу. Первый поиск
  уверенно выдал противоположное; спасло открытие первоисточника.
