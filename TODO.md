...
- [ ] **RAG auto-inject** (опц., после обкатки `search_memory`) — авто-подмешивать top-K релевантных из истории в контекст перед ответом, без явного вызова тула. Сейчас только tool-based (Кеша сам решает). Решить после наблюдения как Кеша пользуется `search_memory`
