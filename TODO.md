...
- [ ] **Логи Codex-рантайма не попадают в journal (#V-37, побочная находка)** — `config.py:51` настраивает только логгер `kesha`, а `codex_session.py:39` берёт `logging.getLogger("codex_session")`; у корневого логгера обработчика нет, поэтому весь INFO Codex теряется. `grep "Codex: starting app-server"` даёт ноль не потому, что рантайм не стартовал. Мешает диагностике блоков
