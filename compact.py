...
async def compact_session(
    ...
    notify=None,
    *,
    allow_native_recovery: bool = False,
) -> dict:
    ...
    try:
        ...
        if failure_reason == "context_limit" and allow_native_recovery:
            native = await claude.run_native_manual_compact(COMPACT_PROMPT)
            if not native.get("ok"):
                failure_reason = f"native_{native.get('reason', 'error')}"
        ...
        elif native_recovered:
            terminal = (
                "⚠️ Контекст сжат встроенной командой, но новое саммари не создано. "
                "Текущая сессия сохранена."
            )
        elif failure_reason == "context_limit":
            terminal = "⚠️ Контекст заполнен — автоматическое сжатие пропущено."
        else:
