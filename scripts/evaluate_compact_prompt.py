...
async def force_real_context_rejection(session: ClaudeSession) -> None:
    oversized = (
        "SYNTHETIC HARD-CONTEXT PROBE. Return no content. "
        + "x " * 1_100_000
    )
    observed = False
    async for chunk in session.send_message(oversized):
        if chunk.get("kind") == "context_limit":
            observed = True
    del oversized
    if not observed:
        raise RuntimeError("hard_context_not_reproduced")


class RecoveryHarness:
    def __init__(self, session: ClaudeSession):
        object.__setattr__(self, "session", session)
        object.__setattr__(self, "inject_observed_limit", True)
    ...
    def __setattr__(self, name, value):
        if name in {"session", "inject_observed_limit", "sent"}:
    ...
    async def send_message(self, text):
        ...
        if self.inject_observed_limit:
            self.inject_observed_limit = False
            yield {
                "type": "error",
                "kind": "context_limit",
                "content": "observed target-runtime hard context rejection",
            }
            return
        async for chunk in self.session.send_message(text):
            yield chunk


async def hard_context_generation(case: dict, root: Path) -> tuple[str, str | None]:
    session = EvaluationSession(
        cwd=str(root),
        model=MODEL,
        system_prompt=system_prompt(case),
    ...
    harness = RecoveryHarness(session)
    try:
        await force_real_context_rejection(session)
        result = await compact_session(
            harness,
            allow_native_recovery=True,
        )
        if not result.get("ok"):
            return "", str(result.get("reason") or "recovery_failed")
        preamble = next(
            (
                text
                for text in harness.sent
                if text.startswith("[PREVIOUS CONTEXT SUMMARY")
            ),
            "",
        )
        if not preamble:
            return "", "missing_recovery_preamble"
        prefix, suffix = CONTINUATION_PREAMBLE.split("{summary}")
        return preamble[len(prefix): -len(suffix)], None
    finally:
        await disconnect(session)
...
async def evaluate_one(case: dict, run: int) -> tuple[dict, list[str]]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"compact-{case['id']}-{run}-") as raw:
        ...
        try:
            if rendered.get("requires_hard_context_recovery"):
                summary, reason = await hard_context_generation(rendered, root)
            else:
                summary, reason = await normal_generation(rendered, root)
            ...
            entry = {
                ...
                "hard_context_recovery": bool(
                    rendered.get("requires_hard_context_recovery")
                ),
            }
