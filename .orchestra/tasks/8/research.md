...
> ## ⚠️ ЧИСЛА MARGIN В ЭТОМ ДОКУМЕНТЕ НЕ ВОСПРОИЗВОДЯТСЯ (проверено 03.08.2026)
>
> Заявленные ниже `avg margin +0.237 / min +0.180` **не подтверждаются на той же модели**.
> Независимо поймала команда Orchestra (#133), я перепроверил на боевом эмбеддере Кеши:
>
> | Триплет «что ел КБЖУ» | в документе | факт (`rag._get_embedder()`, прод) |
> |---|---|---|
> | cos(query, релевантное) | 0.47–0.74 | **0.2213** |
> | cos(query, дистрактор «проблемы с VPN») | 0.27–0.33 | **0.2093** |
> | margin | ≥ +0.180 | **+0.0120** |
>
> Причина: в текст попали ПЕРЕПИСАННЫЕ формулировки триплетов, а мерили на других.
> Скрипты лежали в `/tmp/rag-bench/` и удалены — диффнуть не с чем.
>
> **Что НЕ отменяется:** вывод «bge-m3 лучше E5» подтверждён независимо (Orchestra, 14 триплетов:
> bge-m3 +0.1698 против ada-002 +0.0486, t=−7.49). Решение перейти на bge-m3 было верным —
> ошибочны только абсолютные значения margin. Модель менять не нужно.
>
> **Урок методики:** скрипты замера хранить в репозитории, а не в `/tmp`. Иначе цифру нельзя
> ни воспроизвести, ни опровергнуть — а ссылаться на неё будут.
> Сырые данные и методика Orchestra: `docs/tasks/133/bench/` в их репозитории.
