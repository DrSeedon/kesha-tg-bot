...
- **`ourPrice`** = оценка цены с Ozon-аккаунтом = `round(cardPrice × 0.8949)` (без логина). ПРИБЛИЖЕНИЕ (замер 3 SKU, ±0.1%), не точная цена. `price` = публичная «С банками». Точная аккаунт-цена требует логина (не делаем). Причина: аноним MCP vs залогиненный `premiumSubscribe` tier.
...
- **Cleanup temp-мусора (3 слоя):** Playwright плодит `/tmp/playwright-artifacts-*` + `/tmp/playwright_chromiumdev_profile-*` на каждый запуск браузера. (1) нормальный `browser.close()` сам удаляет профиль (проверено: 5 запросов → /tmp не растёт); (2) `clean-tmp.sh` в wrapper'е сносит orphan'ов >60мин при каждом старте MCP (ловит SIGKILL/обрыв-SSH, когда close() не отработал); (3) `ozon-tmp-cleanup.timer` (systemd, daily 00:00 UTC) — defense-in-depth. Живой браузер idle-close'ится через 10 мин → профиль <60мин никогда не тронут. `.cache/ms-playwright` (641MB) = бинарники браузера (chromium-1223 377M + headless_shell 260M), НЕ рантайм-кеш, не трогать (можно снести избыточный chromium-1223 — репа юзает только headless_shell — но рискованно, оставлено).
