...
- Этот проект — тот самый бот через который Максим общается с Кешей в Telegram
- CWD бота = `/mnt/data/Рабочий стол/Cursor/COG-second-brain` — там живёт Кеша
- Бот запущен как systemd сервис `kesha-bot`
- После правок в коде — `sudo systemctl restart kesha-bot`
- MCP тулы бота доступны как `mcp__kesha__*` (get_bot_status, restart_bot, set_model, set_debounce, toggle_debug)
