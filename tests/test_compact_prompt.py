...
class ScriptedSession(ClaudeSession):
    def __init__(self, session_file: Path, scripts, native=None):
        super().__init__(cwd=".", session_file=session_file)
        self.scripts = list(scripts)
        self.sent = []
        self.native = native or {"ok": True, "trigger": "manual"}
        self.native_calls = []
        self.usage_calls = 0
    ...
    async def run_native_manual_compact(self, instructions):
        self.native_calls.append(instructions)
        return self.native
...
@pytest.mark.asyncio
async def test_manual_context_limit_uses_one_native_boundary_then_commits(tmp_path):
    path = session_file(tmp_path)
    session = ScriptedSession(
        path,
        [
    ...
    result = await compact_session(session, allow_native_recovery=True)

    assert result["ok"] is True
    assert session.native_calls == [COMPACT_PROMPT]
    assert path.read_text() == "sid-candidate"
    assert len(session.sent) == 3


@pytest.mark.asyncio
async def test_automatic_context_limit_never_uses_native_fallback(tmp_path):
    path = session_file(tmp_path)
    session = ScriptedSession(
        path,
        [[{"type": "error", "kind": "context_limit", "content": "Prompt is too long"}]],
    ...
    result = await compact_session(session, allow_native_recovery=False)

    assert result["reason"] == "context_limit"
    assert session.native_calls == []
    assert path.read_text() == "sid-old"


@pytest.mark.asyncio
async def test_native_failure_preserves_original_sid_and_does_not_loop(tmp_path):
    path = session_file(tmp_path)
    session = ScriptedSession(
        path,
        [[{"type": "error", "kind": "context_limit", "content": "Prompt is too long"}]],
        native={"ok": False, "reason": "missing_manual_boundary"},
    )

    result = await compact_session(session, allow_native_recovery=True)

    assert result["reason"] == "native_missing_manual_boundary"
    assert len(session.native_calls) == 1
    assert len(session.sent) == 1
    assert path.read_text() == "sid-old"


@pytest.mark.asyncio
async def test_native_success_retry_failure_keeps_same_compacted_sid(tmp_path):
    path = session_file(tmp_path)
    session = ScriptedSession(
        path,
        [
    ...
    assert session.session_id == "sid-old"
    assert path.read_text() == "sid-old"
    assert "встроенной командой" in notices.items[-1][0]
