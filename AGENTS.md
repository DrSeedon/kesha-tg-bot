...
- **Полный сьют гонять на версиях ПРОДА, а не на «самых свежих».** Изолированный прогон (`uv run --isolated --no-project --with-requirements requirements.txt`) обязан нести `--with 'mcp==1.28.1' --with 'claude-agent-sdk==0.2.158'` — ровно то, что стоит в `/opt/kesha-bot/.venv` (на 23.09.2026 — SDK 0.2.158: в SDK вшит свой Claude Code, и новая модель требует его версию — Opus 5.5 не шёл на встроенном 2.1.259, нужен ≥2.1.280) (сверять `.venv/bin/python -c "import importlib.metadata as m; print(m.version('mcp'))"`). 04.09.2026: `--exclude-newer 2030-01-01` (обход протухшего глобального пина uv) подтянул mcp, где у `Server` больше нет `list_tools` → `kesha_mcp_proxy.py:112` падал на импорте и рушил ВСЮ коллекцию, включая тесты, к зависимости не относящиеся. Это не регрессия задачи: воспроизводится на чистом `main`, а прод при этом жив
- **Под `.orchestra/tasks/<id>/` лежит ЗАПУСКАЕМЫЙ код, и сьют его не покрывает.** Меняешь сигнатуру общего конструктора/протокола — грепай по всему репозиторию, включая каталоги доказательств, а не только корень и `tests/`. 06.09.2026: `ChatRegistry(mcp_config=)` → `mcp_loader=` оставил `.orchestra/tasks/19/probe_argv.py` (боевой acceptance-пробник, запускается руками) с `TypeError`; 636 зелёных тестов этого не видели
- **`codex_review` читает `.orchestra/project-context.toml` из ГЛАВНОГО чекаута, не из worktree воркера.** Нет файла → отказ `project_context_missing` ДО запуска модели, квота не тратится, ревью не состоялось. Воркер починить это не может (путь вне его worktree) и обязан отдать блокер оркестратору; оркестратор кладёт файл в `/home/kesha/projects/kesha-tg-bot/.orchestra/` и коммитит. Общий случай: инструмент требует файл вне твоего worktree → создай его у себя в ветке, повтори вызов ОДИН раз для подтверждения, дальше эскалируй, но в чужой чекаут не пиши
- **review-квитанцию для `merge_worker` может выписать ТОЛЬКО сессия воркера.** Гейт ищет её по `scope+session_id+task_id+target_sha+production_snapshot` (`orchestra/app/review_coverage.py:118-129`), где `session_id` — воркера. `codex_review(mode="implementation")` из scope оркестратора пинит дифф СВОЕЙ головы (`main`) → квитанция уходит с пустым production-диффом и гейт её не принимает. Воркер заблокирован квотой Claude → `change_worker_model` на luna и одним сообщением поручить ему ревью; свой «skip» вместо состоявшегося ревью не выписывать
